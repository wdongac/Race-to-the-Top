...
def main(argv):
	for graph in graphs:
		...
		elif graph == "network10" or graph == "network19":
			thetas = [512.0, 256.0, 128.0, 64.0, 32.0, 16.0, 8.0, 4.0, 2.0]
		else:
		...
		for i in range(len(ks)):
			...
			for epsilon in epsilons:
				...
				for theta in thetas:
					...
					cur_path = os.getcwd()
					cmd = cur_path + "/../../jr_python " + cur_path + "/../Code/NT.py -I " + cur_path + "/../Information/Graph/one_path/" + graph + ".txt -e " + str(epsilon) + " -t " + str(int(theta)) + " -k " + str(i) + " -D " + graph + "_nt"
