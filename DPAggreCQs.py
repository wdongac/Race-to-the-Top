...
def RunAlgorithm():
    ...
    for ii in range(max_i):
        ...
        tilde_Q_tau[i] = hat_Q_tau[i]-tau/epsilon*max_i*math.log(max_i/beta,math.e)*(approximate_factor+1)
        primals[i] = 0
    ...
    for i in range(processor_num):
        ...
        threads.append(multiprocessing.Process(target=ThresholdRunAlgorithm, args=(epsilon,beta,base, downward_sensitivity, real_query_result, arrangement_of_tau_ids[i], 0, tilde_Q_tau)))    
        threads.append(multiprocessing.Process(target=ThresholdRunAlgorithm, args=(epsilon,beta,base, downward_sensitivity, real_query_result, arrangement_of_tau_ids[i], 1, tilde_Q_tau)))    
        threads[i*2].start()
        threads[i*2+1].start()
    for i in range(processor_num):
        threads[i*2].join()
    ...
    for i in range(1,max_i+1):
        ...
        print(i)
        print(early_stops[i])
        print(str(Q_tau[i])+" "+str(tilde_Q_tau[i])+" "+str(hat_Q_tau[i]))
        ...
        if tilde_Q_tau[i]>max_val:
            ...
            max_ind = i
    final_res = hat_Q_tau[max_ind]
    return final_res



def ThresholdRunAlgorithm(epsilon,beta, base, downward_sensitivity, real_query_result, assigned_of_tau_ids, LP_type, tilde_Q_tau):
...
def main(argv):
    ...
    try:
        ...
        print("DPAggreCQs.py -I <input file> -e <epsilon(default 0.1)> -b <beta(default 0.01)> -G <global sensitivity(default 1000,000)> -p <processor number(even number, default 2)> -A <approximate Factor(default 0)>")
        sys.exit(2)
    for opt, arg in opts:
        if opt == '-h':
            print("DPAggreCQs.py -I <input file> -e <epsilon(default 0.1)> -b <beta(default 0.01)> -G <global sensitivity(default 1000,000)> -p <processor number(even number, default 2)> -A <approximate Factor(default 0)>")
